...
def find_or_create_playlist(sp, username, user_playlists, playlist_name):
    ...
    playlistfound = list(filter(lambda d: d['name'] in [
                         f'{playlist_name}'], user_playlists))
    ...
    with open(csv_file, newline='') as csv_file:
        ...
        for row in song_reader:
            ...
            if response:
                ...
                print("Found {0} by {1}".format(
                    first_song['name'], first_song['album']['artists'][0]['name']))
                if counter % 100 == 0:
                    sp.user_playlist_add_tracks(
                        username, playlist_id, song_ids)
                    song_ids = []
                    counter = 0
            else:
...
def add_songs_from_csv_genre(sp, csv_file, username, playlists_by_genre, playlist_hash, songs_not_found):
    with open(csv_file, newline='') as csv_file:
        ...
        for row in song_reader:
            response = find_song(sp, row[0], row[1])
            if response:
                playlist_key = playlists_by_genre[row[2]
                                                  ] if row[2] in playlists_by_genre else None
                if playlist_key:
                    first_song = response[0]
                    playlist_hash[playlist_key]['song_ids'].append(
                        first_song['id'])
                    print("Found {0} by {1} for playlist {2}".format(
                        first_song['name'], first_song['album']['artists'][0]['name'], playlist_key))
                    if len(playlist_hash[playlist_key]['song_ids']) % 100 == 0:
                        sp.user_playlist_add_tracks(
                            username, playlist_hash[playlist_key]['playlist_id'], playlist_hash[playlist_key]['song_ids'])
        ...
        for playlist in playlist_hash:
            if len(playlist_hash[playlist]['song_ids']) > 0:
                sp.user_playlist_add_tracks(
                    username, playlist_hash[playlist]['playlist_id'], playlist_hash[playlist]['song_ids'])
...
def parse_playlists_from_genre(sp, username, user_playlists, playlists_by_genre):
    ...
    playlist_hash = {playlist: {"playlist_id": None, "song_ids": []}
                     for playlist in playlists}
    for playlist in playlist_hash.keys():
        playlist_hash[playlist]['playlist_id'] = find_or_create_playlist(
            sp, username, user_playlists, playlist)
