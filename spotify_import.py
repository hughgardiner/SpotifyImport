...
parser.add_argument('-g', '--genre', type=str,
                    help='Include yaml file categorizing subgenres into playlists')
...
if args.genre is not None:
    ...
    with open(args.genre, 'r') as genre_file:
        playlists_by_genre = create_genre_playlist_dict(
            yaml.safe_load(genre_file))
        if type(playlists_by_genre) is not dict:
            invalid_genre_file()
        playlist_hash = parse_playlists_from_genre(
            sp, args.username, user_playlists, playlists_by_genre)
    if playlist_hash:
        add_songs_from_csv_genre(sp, args.filename, args.username,
